...
// Add a flag to track if we're in the perfect hit state
let perfectHitActive = false;
      ...
      if (sticks.last().rotation > 90) {
        ...
        if (nextPlatform) {
          ...
          // perfectElement.innerText = randomMessage; // Display the message
           ...
           // Set the perfect hit state to active
           perfectHitActive = true;

            // Wait for the space bar to be pressed to continue
           window.addEventListener("keydown", handleSpacebarForPerfectHit);
          }
...
// Handle space bar press to continue the game after a perfect hit
function handleSpacebarForPerfectHit(event) {
  if (event.key == " " && perfectHitActive) {
    perfectHitActive = false; // Reset the perfect hit state
    const messageBox = document.getElementById("perfectHitMessageBox");
    messageBox.style.display = "none"; // Hide the message box
    window.removeEventListener("keydown", handleSpacebarForPerfectHit); // Remove the event listener
  }
}
